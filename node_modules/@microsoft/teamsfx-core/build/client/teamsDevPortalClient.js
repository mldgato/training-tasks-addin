...
class TeamsDevPortalClient {
        ...
        try {
            ...
            if (response && response.data) {
                ...
                if (app && app.teamsAppId && app.teamsAppId === teamsAppId) {
                    ...
                    globalVars_1.TOOLS.logProvider?.error(`teamsAppId mismatch. Input: ${teamsAppId}. Got: ${app.teamsAppId}`);
                }
            }
        }
        catch (e) {
    ...
    async getBotId(token, teamsAppId) {
        ...
        globalVars_1.TOOLS.logProvider?.error(`botId not found. Input: ${teamsAppId}`);
        return undefined;
    }
    ...
    checkUser(app, newUser) {
        const findUser = app.userList?.findIndex((user) => user["aadId"] === newUser.aadId);
    ...
    handleBotFrameworkError(e, apiName) {
        ...
        else if (e.response?.status === commonConstant_1.HttpStatusCode.UNAUTHORIZED) {
            throw new errors_2.BotFrameworkNotAllowedToAcquireTokenError();
        }
        else if (e.response?.status === commonConstant_1.HttpStatusCode.FORBIDDEN) {
            throw new errors_2.BotFrameworkForbiddenResultError(e);
        }
        else if (e.response?.status === commonConstant_1.HttpStatusCode.TOOMANYREQS) {
    ...
    wrapResponse(e, response) {
        const error = new Error(e?.message || response?.data.error?.message || response?.data.errorMessage);
