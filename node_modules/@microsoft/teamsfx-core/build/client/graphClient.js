...
class GraphClient {
    baseUrl = process.env.GRAPH_ENDPOINT ?? `${(0, constants_1.getResourceServiceEndpoint)(constants_1.ResourceServiceType.Graph)}/beta`;
    tokenProvider;
    logProvider;
    ...
    async getGroupInfo(email) {
        ...
        const group = res.data.value.find((group) => group.mail?.toLowerCase() === email.toLowerCase());
        if (!group) {
            return undefined;
        }
        return group;
