            ...
            if (mcpTools && mcpTools.length > 0 && mcpToolsSelected.length > 0) {
                ...
                if (mcpAuth === "OAuthPluginVault") {
                    ...
                    try {
                        ...
                        let refreshUrl;
                        if (authType === "oauth") {
                            const { resolveMCPOAuthMetadata } = await Promise.resolve().then(() => tslib_1.__importStar(require("../component/utils/mcpToolFetcher")));
                            const metadata = await resolveMCPOAuthMetadata(inputs[constants_2.QuestionNames.MCPForDAAuthMetadataUrl]);
                            authorizationUrl = metadata.authorizationUrl;
                            tokenUrl = metadata.tokenUrl;
                            refreshUrl = metadata.refreshUrl;
                        }
                        const { ActionInjector } = await Promise.resolve().then(() => tslib_1.__importStar(require("../component/configManager/actionInjector")));
                        const ymlPath = pathUtils_1.pathUtils.getYmlFilePath(inputs.projectPath);
                        if (ymlPath) {
                            await ActionInjector.injectCreateOAuthActionForMCP(ymlPath, authType, actionId, registrationId, mcpServerUrl, authorizationUrl, tokenUrl, refreshUrl);
                        }
                    }
                    catch (error) {
                        mcpWarnings.push({
